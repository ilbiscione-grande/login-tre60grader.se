  ...
  select
    ...
    case
      ...
      when exists (
        select 1
        from company_memberships cm
        where cm.role in ('employee', 'member', 'finance', 'auditor')
      ) then 'employee'
      when exists (
        select 1
  ...
  jsonb_build_object(
    ...
    'notes', jsonb_strip_nulls(
      jsonb_build_object(
        ...
        'staff_role_compat', case when c.derived_role = 'employee' and exists (select 1 from company_memberships cm where cm.role in ('finance', 'auditor')) then 'company_members.role = finance/auditor mappas till intern employee-kompatibilitet i auth-lagret' end,
